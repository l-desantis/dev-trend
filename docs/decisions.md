...
**Implementation note — merge-pass divergence from spec §4.3:** Spec §4.3 describes the merge pass as: re-cluster from scratch, then for each new cluster find the best-matching active candidate; if multiple candidates map to the same new cluster, merge them. The v1 implementation in `app/pipeline/recluster.py` uses an all-pairs centroid comparison instead (compare every pair of active candidates; merge those whose centroids exceed the similarity threshold). This produces equivalent outcomes when candidate centroids are close (the common case), but does not use the new cluster groupings as the merge criterion. The all-pairs approach is kept because it has full test coverage and the spec behaviour is an improvement for a future plan (v4.D or later).
